...
void QNetworkReplyHandler::sendResponseIfNeeded()
{
    ...
    if (url.protocolIsInHTTPFamily()) {
        ...
        response.setHTTPStatusText(std::span { status.constData(), static_cast<size_t>(status.length()) });
        ...
        Q_FOREACH (const QNetworkReply::RawHeaderPair& pair, m_replyWrapper->reply()->rawHeaderPairs())
            response.setHTTPHeaderField(std::span { pair.first.constData(), static_cast<size_t>(pair.first.length()) },
                                        makeStringByReplacingAll(std::span { pair.second.constData(), static_cast<size_t>(pair.second.length()) }, '\n', ','));
    }

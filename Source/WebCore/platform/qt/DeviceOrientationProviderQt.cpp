...
bool DeviceOrientationProviderQt::filter(QRotationReading* reading)
{
    if (m_controller) {
        ...
        // FIXME: verify that QRotationReading provides absolute readings rather than relative ones
        m_lastOrientation = DeviceOrientationData::create(reading->z(), reading->x(), reading->y(), true);
